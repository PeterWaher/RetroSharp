...
namespace PeerToPeer
{
	...
	class Program : RetroApplication
	{
		public static void Main(string[] args)
		{
			...
			try
			{
				Dictionary<IPAddress, bool> IPAddressesFound = new Dictionary<IPAddress, bool>();

				using (UPnPClient UPnP = new UPnPClient())
				{
					UPnP.OnDeviceFound += (sender, LocationEventArgs) =>
					{
						bool Found;

						lock (IPAddressesFound)
						{
							if (!(Found = IPAddressesFound.ContainsKey(LocationEventArgs.RemoteEndPoint.Address)))
								IPAddressesFound[LocationEventArgs.RemoteEndPoint.Address] = true;
						}
								...
								if (DeviceEventArgs.DeviceDescriptionDocument != null)
								{
									...
									if (Service != null)
									{
										Service.StartGetService((sender3, ServiceEventArgs) =>
										{
											...
											// TODO: First check if one exists.
